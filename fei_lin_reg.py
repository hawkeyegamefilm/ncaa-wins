...
all_files = glob.glob("fei-data/*.csv")
...
all_data['Wins'] = all_data.apply(lambda row: int(str(row.Rec).split('-')[0]), axis=1)

#Use FEI as our predictor and we're trying to predict Wins
x = all_data.FEI.values.reshape(-1, 1)
...
plt.xticks((-2,-1.5,-1.0,-.5,0,.5,1.0,1.5,2))
plt.yticks((0,2,4,6,8,10,12,14,16))
plt.ylim([-1,16])
plt.title("NCAA Wins as a function of FEI")
plt.xlabel("FEI")
plt.ylabel("Wins")
plt.show()
